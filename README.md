...
DEFAULT_YT_COOKIE=/path/to/cookie_wallen.txt
...
├── worker.py               # Функции скачивания медиа
├── bot_commands.py         # Обработчики команд бота
├── inline_commands.py      # Inline режим
├── link_handler.py         # Обработка ссылок
├── pinterest.py            # Pinterest загрузчик
├── data/
│   └── config.py           # Конфигурация
├── db/
│   ├── db_commands.py      # Команды БД
│   ├── audio_commands.py   # Аудио команды
│   ├── audio_helper.py     # Помощники для аудио
│   └── download_log.py     # Логирование загрузок
├── models/
│   ├── base.py             # Базовая модель
│   ├── users.py            # Модель пользователей
│   ├── audio.py            # Модель аудио
│   └── download_log.py     # Модель логов загрузок
├── migration/
│   └── versions/           # Alembic миграции
├── mini-app/               # Vue.js Mini App
│   ├── src/
...
Создайте `/etc/systemd/system/media_helper_bot.service`:

```ini
[Unit]
Description=Media Helper Bot
After=network.target postgresql.service

[Service]
Type=simple
User=root
WorkingDirectory=/root/media_helper
Environment=PATH=/root/media_helper/myvenv/bin
ExecStart=/root/media_helper/myvenv/bin/python main.py
Restart=always
RestartSec=10
...
sudo systemctl enable media_helper_bot
sudo systemctl start media_helper_bot
```

### Nginx конфиг для Mini App
...
server {
    ...
    location / {
        root /root/media_helper/mini-app/dist;
